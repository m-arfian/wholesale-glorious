...
class CetakController extends Controller {
    ...
    public function actionNota($kode) {
        $order = Order::model()->findByPk($kode);
        if($order === null)
            throw new CHttpException('Halaman yang Anda cari tidak ditemukan', 404);
        
        $mPDF1 = Yii::app()->ePdf->mpdf('', 'A5');
        
        $mPDF1->WriteHTML(file_get_contents(Yii::app()->theme->basePath . '/css/bootstrap.min.css'), 1);
        $mPDF1->WriteHTML(file_get_contents(Yii::getPathOfAlias('webroot.css') . '/main.css'), 1);

        # renderPartial (only 'view' of current controller)
        $mPDF1->WriteHTML($this->renderPartial('_nota_invoice', array('order' => $order), true));
        
        # Outputs ready PDF
        $mPDF1->Output($kode . '-Cetak-Nota' . '.pdf', 'D');
    }
